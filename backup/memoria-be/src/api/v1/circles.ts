 ...
 * Circles API Routes
 *
 * Handles circle CRUD, photo sharing, and member management
 */
...
router.post('/:id/photos/bulk', async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const userId = req.user!.id;
    ...
    circlePhotos.map(async (cp) => {
      ...
      return {
        ...
        uploadedBy: {
          ...
          profilePicUrl: cp.uploader.profilePicUrl,
        },
  ...
  res.json({
    member: {
      ...
      user: {
        ...
        profilePicUrl: updatedMember.user.profilePicUrl,
      },
